...
class NotificationsToEntitiesRepository extends AbstractRepository
{
    ...
    public function getEntities($notificationId)
    {
        try {
            ...
            $entityRows = $statement->fetchAll(\PDO::FETCH_COLUMN);
    ...
    public function removeEntity($notificationId, $entityId, $entity)
    {
        ...
        try {
            $statement = $this->connection->prepare(
                "DELETE FROM {$this->table} WHERE notificationId = :notificationId AND entityId = :entityId AND entity = :entity"
            );
    ...
    public function addEntity($notificationId, $entityId, $entity)
    {
        ...
        try {
            $statement = $this->connection->prepare(
                "INSERT INTO {$this->table} (`notificationId`, `entityId`, `entity`) VALUES (:notificationId, :entityId, :entity)"
            );
