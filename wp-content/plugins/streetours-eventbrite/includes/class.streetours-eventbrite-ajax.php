...
if ( ! class_exists( 'streetours_eventbrite_ajax' ) ) {

    class streetours_eventbrite_ajax {
        ...
        public function __construct() {
            ...
            $this->api_app = get_option('streetours-eventbrite-app-key') ?? "646cb627-293d-41dc-9f69-3d5cb25ec2fc";
            $this->api_user = get_option('streetours-eventbrite-user-key') ?? "c7ebe150-95e2-4dba-bb0b-64c3022ec99b";
            $this->company = get_option('streetours_eventbrite_company') ?? 'bodyglove';
            $this->root_url = 'https://demo.eventbrite.com/api/external/v1';
        ...
        public function checkAbilityOfSelectedDateAction() {
            ...
            wp_verify_nonce( 'streetours_eventbrite_nonce', 'security' );

            $date = $_REQUEST['date'];


            $availabilities_url = "$this->root_url/companies/$this->company/items/$this->item/minimal/availabilities/date/$date/";
            //https://demo.eventbrite.com/api/external/v1/companies/bodyglove/items/183/minimal/availabilities/date/2021-12-01
//            $response = \Httpful\Request::get($availabilities_url)
//                ->addHeader("X-Eventbrite-API-App", $api_app)
//                ->addHeader("X-Eventbrite-API-User", $api_user)
//                ->send();

            $response_encoded = wp_remote_get($availabilities_url, array(
                'headers' => array(
                    'X-Eventbrite-API-App' => $this->api_app,
                    'X-Eventbrite-API-User'=> $this->api_user
                ),
            ));
